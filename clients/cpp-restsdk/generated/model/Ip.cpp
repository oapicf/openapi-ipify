...
bool Ip::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("ip"))))
    {
        ...
        {
            utility::string_t refVal_ip;
            ok &= ModelBase::fromJson(fieldValue, refVal_ip);
            setIp(refVal_ip);
        }
    }
    return ok;
...
bool Ip::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        utility::string_t refVal_ip;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("ip"))), refVal_ip );
        setIp(refVal_ip);
    }
    return ok;
}
