...
public class DefaultApiVerticle extends AbstractVerticle {
    ...
    static final String GETIP_SERVICE_ID = "getIp";
    ...
    public void start() throws Exception {
        ...
        //Consumer for getIp
        vertx.eventBus().<JsonObject> consumer(GETIP_SERVICE_ID).handler(message -> {
            try {
                // Workaround for #allParams section clearing the vendorExtensions map
                String serviceId = "getIp";
                ...
                service.getIp(format, paramCallback, result -> {
                    if (result.succeeded())
                        message.reply(new JsonObject(Json.encode(result.result())).encodePrettily());
                    else {
                        Throwable cause = result.cause();
                        manageError(message, cause, "getIp");
                    }
                });
            } catch (Exception e) {
                logUnexpectedError("getIp", e);
