    ...
    fn call(&self, req: (Request<ReqBody>, C)) -> Self::Future {
            ...
            ReqBody::Data: Send,
        {
            ...
            hyper::Method::GET if path.matched(paths::ID_) => {
                handle_get_ip(api_impl, uri, headers, body, context, validation).await
            },
...
async fn handle_get_ip<T, C, ReqBody>(
    mut api_impl: T,
    uri: hyper::Uri,
    headers: HeaderMap,
    body: ReqBody,
    ...
    ReqBody::Data: Send,
{
                ...
                let param_format = match param_format {
                    Some(param_format) => {
                        let param_format =
                            <models::GetIpFormatParameter as std::str::FromStr>::from_str
                                (&param_format);
                        match param_format {
    ...
    #[cfg(not(feature = "validate"))]
                run_validation!(param_format, "format", validation);
                ...
                let param_callback = match param_callback {
                    Some(param_callback) => {
                        ...
                        match param_callback {
                            Ok(param_callback) => Some(param_callback),
                            Err(e) => return Ok(Response::builder()
                                .status(StatusCode::BAD_REQUEST)
                                .body(body_from_string(format!("Couldn't parse query parameter callback - doesn't match schema: {e}")))
                                .expect("Unable to create Bad Request response for invalid query parameter callback")),
                        }
                    },
                    None => None,
                };

                                let result = api_impl.get_ip(
                                            param_format,
                                            param_callback,
                                        &context
                                    ).await;
    ...
    match body {
        swagger::OneOf3::<String, models::GetIp200Response, String>::A(body) => {
                                                    response.headers_mut().insert(
                                                        CONTENT_TYPE,
        ...
        swagger::OneOf3::<String, models::GetIp200Response, String>::B(body) => {
                                                    response.headers_mut().insert(
                                                        CONTENT_TYPE,
        ...
        swagger::OneOf3::<String, models::GetIp200Response, String>::C(body) => {
                                                    response.headers_mut().insert(
                                                        CONTENT_TYPE,
