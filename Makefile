...
generate:
	if [ "${LOCAL}" = "true" ]; then \
	  make  generate-langs GEN_BASE_DIR=/Users/cliffano/dev/workspace-studio/openapi-ipify; \
	elif [ "${GITHUB_ACTIONS}" = "true" ]; then \
	  make generate-langs GEN_BASE_DIR=/home/runner/work/openapi-ipify/openapi-ipify; \
	fi
...
generate-langs:
	for lang in ${LANGS} ; do \
	  docker \
		  ...
		  generate \
		  --input-spec /local/specification/ipify.yml \
...
generate-primary:
	if [ "${LOCAL}" = "true" ]; then \
	  make  generate-langs-primary GEN_BASE_DIR=/Users/cliffano/dev/workspace-studio/openapi-ipify; \
	elif [ "${GITHUB_ACTIONS}" = "true" ]; then \
	  make generate-langs-primary GEN_BASE_DIR=/home/runner/work/openapi-ipify/openapi-ipify; \
	fi
...
doc-version:
	bootprint openapi specification/ipify.yml doc/api/$(version)/
