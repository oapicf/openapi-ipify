...
class Decoders {
    ...
    private static var __once: () = {
        ...
        // Decoder for [Ip]
        Decoders.addDecoder(clazz: [Ip].self) { (source: AnyObject, instance: AnyObject?) -> Decoded<[Ip]> in
            return Decoders.decode(clazz: [Ip].self, source: source)
        }

        // Decoder for Ip
        Decoders.addDecoder(clazz: Ip.self) { (source: AnyObject, instance: AnyObject?) -> Decoded<Ip> in
            if let sourceDictionary = source as? [AnyHashable: Any] {
                let _result = instance == nil ? Ip() : instance as! Ip
                ...
                return .failure(.typeMismatch(expected: "Ip", actual: "\(source)"))
            }
        }
    }()
