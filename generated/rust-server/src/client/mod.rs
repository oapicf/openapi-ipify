...
impl<F, C> Api<C> for Client<F> where
    ...
    fn get_ip(&self, param_format: Option<String>, param_callback: Option<String>, context: &C) -> Box<Future<Item=GetIpResponse, Error=ApiError>> {

        // Query parameters
        let query_format = param_format.map_or_else(String::new, |query| format!("format={format}&", format=query.to_string()));
        let query_callback = param_callback.map_or_else(String::new, |query| format!("callback={callback}&", callback=query.to_string()));
        ...
        let uri = format!(
            ...
            callback=utf8_percent_encode(&query_callback, QUERY_ENCODE_SET)
        );
